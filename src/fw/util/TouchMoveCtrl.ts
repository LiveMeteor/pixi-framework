...
namespace fw {
	...
	export class TouchMoveCtrl {
		...
		protected beginPoint?: PIXI.Point;
		protected nowPoint?: PIXI.Point;
		...
		protected onTouchScroll(evt: PIXI.interaction.InteractionEvent): void {
			if (evt.type == "pointerdown") {
				...
				// this.beginPoint = evt.data.global.clone();
				...
				if (this.bindXYProp) {
					let begin: PIXI.Point = this.bindXYProp.getfunc.call(this.bindXYProp.target);
					this.beginX = begin.x;
					this.beginY = begin.y;
				}
			}
				...
				if (!this.isTouchDown)
					return;
				// this.nowPoint = evt.data.global.clone();
		...
		public bindXChange(target: any, prop: string): void {
			if (typeof (target[prop]) != "number") {
				Log.error(this, "bind property is not number type!!!");
				return;
			}
		...
		/** 绑定 XY 位置变化时触发的方法 */
		public bindXYChange(target: any, getfunc: () => PIXI.Point, setfunc: (posx: number, posy: number) => void): void {
