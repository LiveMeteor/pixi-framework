...
namespace fw {
     ...
     * 声音引擎，使用 原生播放 的方式
     */
    export class NativeAudioEngine extends HashObject implements SoundEngine {
        ...
        public stopSound(name: string): void {
            const result = this.executeStopSound(name);
            result && this.soundMap.remove(name);
        }

        private executeStopSound(name: string): boolean {
            const props = this.soundMap.getValue(name);
            if (props)
            {
                props.event = "stop";
        ...
        public removeAllSound(): void {
            this.soundMap.eachKey(name => {
                this.executeStopSound(name);
            }, this);
            this.soundMap.clear();
        }
